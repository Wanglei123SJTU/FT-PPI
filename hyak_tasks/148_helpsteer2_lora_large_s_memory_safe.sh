...
echo "helpsteer2_lora_large_s_memory_safe_task_start $(date)"
...
RUN_NAME="helpsteer2_lora_large_s_memory_safe_148"
OUTPUT_DIR="/gscratch/scrubbed/${USER}/ft-ppi/artifacts/${RUN_NAME}"
INPUT_CSV="$OUTPUT_DIR/helpsteer2_preference_pairs.csv"
SOURCE_CSV="/gscratch/scrubbed/${USER}/ft-ppi/artifacts/helpsteer2_lora_large_s_diagnostic_146/helpsteer2_preference_pairs.csv"
BASE_MODEL_NAME="${BASE_MODEL_NAME:-Qwen/Qwen2.5-1.5B-Instruct}"
SNAPSHOT_PATH="/gscratch/scrubbed/${USER}/ft-ppi/cache/huggingface/hub/models--Qwen--Qwen2.5-1.5B-Instruct/snapshots/989aa7980e4cf806f80c7fef2b1adb7bc71aa306"
FEATURES="delta_log_length_scale,delta_log_sentences_scale"
TARGETS="delta_log_length_scale,delta_log_sentences_scale"
S_GRID="${S_GRID:-500,1000,1500,2000}"
...
PATIENCE="${PATIENCE:-1}"
VALIDATION_LIMIT="${VALIDATION_LIMIT:-2048}"
EVALUATION_LIMIT="${EVALUATION_LIMIT:-0}"
NUM_WORKERS="${NUM_WORKERS:-8}"
ARRAY_CONCURRENCY="${ARRAY_CONCURRENCY:-$NUM_WORKERS}"
NO_4BIT="${NO_4BIT:-0}"
NO_GRADIENT_CHECKPOINTING="${NO_GRADIENT_CHECKPOINTING:-0}"
...
mkdir -p "$OUTPUT_DIR" "$OUTPUT_DIR/cells" /gscratch/scrubbed/"$USER"/ft-ppi/logs
...
if [ ! -s "$SNAPSHOT_PATH/config.json" ]; then
  echo "snapshot not found; downloading base_model_name=$BASE_MODEL_NAME"
...
test -s "$SNAPSHOT_PATH/config.json"
...
echo "train_batch_size=$TRAIN_BATCH_SIZE grad_accum=$GRAD_ACCUM eval_batch_size=$EVAL_BATCH_SIZE no_4bit=$NO_4BIT no_gradient_checkpointing=$NO_GRADIENT_CHECKPOINTING"
...
echo "submitting memory-safe large-s diagnostic: ${SBATCH_CMD[*]}"
SUBMIT_OUTPUT="$("${SBATCH_CMD[@]}")"
echo "$SUBMIT_OUTPUT"
JOB_ID="$(echo "$SUBMIT_OUTPUT" | awk '{print $NF}')"
echo "memory_safe_large_s_job_id=$JOB_ID"
...
echo "helpsteer2_lora_large_s_memory_safe_task_done $(date)"
