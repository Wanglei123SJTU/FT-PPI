...
echo "helpsteer2_qwen_embedding_mlp_scaling_cache_retry_task_start $(date)"
...
PYTHON_BIN="$(readlink -f "$PYTHON_BIN" 2>/dev/null || command -v "$PYTHON_BIN")"

echo "python=$($PYTHON_BIN --version)"
echo "python_bin=$PYTHON_BIN"

CACHE_BASE="/gscratch/foster/$USER/ft-ppi"
if ! mkdir -p "$CACHE_BASE/cache" 2>/dev/null; then
  CACHE_BASE="/gscratch/scrubbed/$USER/ft-ppi"
  mkdir -p "$CACHE_BASE/cache"
fi
CACHE_ROOT="${HYAK_CACHE_DIR:-$CACHE_BASE/cache}"
mkdir -p "$CACHE_ROOT/huggingface" "$CACHE_ROOT/hf_hub" "$CACHE_ROOT/hf_datasets" "$CACHE_ROOT/torch" "$CACHE_ROOT/xdg"
export HF_HOME="$CACHE_ROOT/huggingface"
export HF_HUB_CACHE="$CACHE_ROOT/hf_hub"
export TRANSFORMERS_CACHE="$CACHE_ROOT/hf_hub"
...
if compgen -G "${SCALING_ROOT}/*_delta_format/mlp_scaling_summary.csv" > /dev/null; then
  echo "existing_scaling_outputs_found"
  for report in ${SCALING_ROOT}/*_delta_format/mlp_scaling_report.md; do
    echo "existing_report=$report"
    sed -n '1,220p' "$report" || true
  done
  echo "helpsteer2_qwen_embedding_mlp_scaling_cache_retry_task_done $(date)"
  exit 0
fi
...
submit_embedding_job() {
  ...
  local env_prefix="export HF_HOME='$HF_HOME' HF_HUB_CACHE='$HF_HUB_CACHE' TRANSFORMERS_CACHE='$TRANSFORMERS_CACHE' HF_DATASETS_CACHE='$HF_DATASETS_CACHE' TORCH_HOME='$TORCH_HOME' XDG_CACHE_HOME='$XDG_CACHE_HOME' TOKENIZERS_PARALLELISM=false PYTORCH_CUDA_ALLOC_CONF='$PYTORCH_CUDA_ALLOC_CONF';"
  ...
  job_id=$(sbatch --parsable $GPU_ARGS \
    ...
    --cpus-per-task=8 \
    --output="$slurm_log" \
    --wrap="cd '$REPO_DIR' && $env_prefix '$PYTHON_BIN' -m src.experiments.helpsteer2_embedding_extraction --input-csv '$PAIR_CSV' --output-npz '$embed_npz' --model-name '$model_name' --batch-size '$batch_size' --max-length '$max_length'")
...
ENV_PREFIX="export HF_HOME='$HF_HOME' HF_HUB_CACHE='$HF_HUB_CACHE' TRANSFORMERS_CACHE='$TRANSFORMERS_CACHE' HF_DATASETS_CACHE='$HF_DATASETS_CACHE' TORCH_HOME='$TORCH_HOME' XDG_CACHE_HOME='$XDG_CACHE_HOME' TOKENIZERS_PARALLELISM=false PYTORCH_CUDA_ALLOC_CONF='$PYTORCH_CUDA_ALLOC_CONF';"
# shellcheck disable=SC2086
mlp_job_id=$(sbatch --parsable $GPU_ARGS \
  ...
  --cpus-per-task=8 \
  --output="$MLP_LOG" \
  --wrap="cd '$REPO_DIR' && $ENV_PREFIX '$PYTHON_BIN' -m src.experiments.helpsteer2_embedding_mlp_scaling --input-csv '$PAIR_CSV' --embedding-npz '$EMBED_NPZ' --output-dir '$MLP_OUT' --target delta_format --s-grid 0,50,100,250,500,750,1000,1500,3000 --replications 10 --hidden-dim 128 --batch-size 64 --max-epochs 300 --patience 30")
