...
echo "zerohead_signal_probe_task_start"
hostname
date
...
echo "== SUBMIT ZERO-HEAD SIGNAL PROBE =="
submit_output=""
submit_status=1
for args in \
...
do
  echo "Trying: sbatch $args slurm/run_training_signal_probe_zerohead.sbatch"
  set +e
  submit_output=$(sbatch $args slurm/run_training_signal_probe_zerohead.sbatch 2>&1)
  submit_status=$?
  set -e
  echo "$submit_output"
...
if [ "$submit_status" -ne 0 ]; then
  echo "All zero-head signal probe submit attempts failed."
  exit 1
fi
...
echo "JOB_ID=$JOB_ID"
JOB_LOG="logs/wine-zerohead-${JOB_ID}.out"
...
root = Path("artifacts/training_signal_probe_zerohead")
summary = root / "summary"
required = [summary / "prediction_diagnostics.csv", summary / "allocation_curve.csv"]
...
if diagnostics.empty or curve.empty:
    raise SystemExit("summary tables are empty")
if diagnostics["variant"].nunique() != 2:
    raise SystemExit("missing variants in diagnostics")

pd.set_option("display.max_columns", None)
pd.set_option("display.width", 220)
print("zero-head diagnostics rows=", len(diagnostics))
print(diagnostics[diagnostics["role"].isin(["population", "correction"])].to_string(index=False))
ppi = curve[curve["method"].astype(str).str.contains("ppi_plus", regex=False)]
print("zero-head ppi_plus")
print(ppi.to_string(index=False))
PY

echo "zerohead_signal_probe_task_done"
