...
echo "upworthy_backup_lowdim_pilots_task_start"
hostname
date
...
required = {"delta_QUESTION", "delta_NUMERIC", "delta_HAS_QUOTES", "delta_CAPS_SHARE", "delta_LENGTH"}
if not path.exists():
...
CONFIGS=(
  "configs/upworthy_question_targetonly_textonly_lowdim_pilot_1p5b.yaml"
  "configs/upworthy_numeric_hasquotes_textonly_lowdim_pilot_1p5b.yaml"
  "configs/upworthy_hasquotes_numeric_textonly_lowdim_pilot_1p5b.yaml"
  "configs/upworthy_capsshare_length_textonly_lowdim_pilot_1p5b.yaml"
)

for cfg in "${CONFIGS[@]}"; do
  echo "== RUN BACKUP LOWDIM PILOT: $cfg =="
  export CONFIG="$cfg"
  bash hyak_tasks/080_upworthy_question_scaling_diagnostic_full.sh
done

echo "upworthy_backup_lowdim_pilots_task_done"
