...
echo "cancel_h200_and_run_helpsteer2_worker_a40_fallback_task_start $(date)"

cd "${HYAK_RUNNER_REPO_DIR:-$PWD}"
git status --short --branch

OLD_JOB_ID="${OLD_JOB_ID:-36268033}"
...
if [ "$old_state" = "PENDING" ]; then
  echo "canceling pending old job $OLD_JOB_ID"
  scancel "$OLD_JOB_ID" || true
elif [ -n "$old_state" ]; then
  echo "old job is not pending; leaving it alone and exiting"
  exit 0
fi
...
OUTPUT_DIR="${OUTPUT_DIR:-/gscratch/scrubbed/${USER}/ft-ppi/artifacts/helpsteer2_lora_worker_quickdiag_a40_fallback}"
...
GPU_ARGS="${HYAK_GPU_ARGS:---partition=ckpt --gres=gpu:a40:1}"
...
echo "submitting fallback: ${SBATCH_CMD[*]}"
SUBMIT_OUTPUT="$("${SBATCH_CMD[@]}")"
echo "$SUBMIT_OUTPUT"
JOB_ID="$(echo "$SUBMIT_OUTPUT" | awk '{print $NF}')"
echo "fallback_job_id=$JOB_ID"
...
echo "cancel_h200_and_run_helpsteer2_worker_a40_fallback_task_done $(date)"
