...
TASK_ID="115_run_question_targetonly_confirm_generic_gpu"
...
CONFIG="configs/upworthy_question_targetonly_confirm_10rep_generic_1p5b.yaml"
ROOT="/gscratch/scrubbed/${USER}/ft-ppi/artifacts/upworthy_question_targetonly_confirm_10rep_generic_1p5b"

echo "== USER GPU QUEUE BEFORE SUBMIT =="
squeue -u "$USER" || true

echo "== DESCRIBE QUESTION TARGET-ONLY CONFIRM CONFIG =="
.venv-hyak/bin/python -m src.experiments.upworthy_question_scaling_law describe --config "$CONFIG"

echo "== RUN QUESTION TARGET-ONLY 10-REP CONFIRMATION ON GENERIC CKPT-G2 GPU =="
export CONFIG
...
echo "== QUESTION TARGET-ONLY GENERIC CONFIRMATION SUMMARY =="
.venv-hyak/bin/python - <<'PY'
from pathlib import Path
import pandas as pd

root = Path("/gscratch/scrubbed/lei0603/ft-ppi/artifacts/upworthy_question_targetonly_confirm_10rep_generic_1p5b")
