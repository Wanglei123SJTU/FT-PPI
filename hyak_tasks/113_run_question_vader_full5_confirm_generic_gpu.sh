...
TASK_ID="113_run_question_vader_full5_confirm_generic_gpu"
...
CONFIG="configs/upworthy_question_vader_full5_confirm_10rep_generic_1p5b.yaml"
ROOT="/gscratch/scrubbed/${USER}/ft-ppi/artifacts/upworthy_question_vader_full5_confirm_10rep_generic_1p5b"

echo "== USER GPU QUEUE BEFORE SUBMIT =="
squeue -u "$USER" || true

echo "== DESCRIBE QUESTION + VADER FULL5 CONFIRM CONFIG =="
.venv-hyak/bin/python -m src.experiments.upworthy_question_scaling_law describe --config "$CONFIG"

echo "== RUN QUESTION + VADER FULL5 10-REP CONFIRMATION ON GENERIC CKPT-G2 GPU =="
export CONFIG
...
echo "== QUESTION + VADER FULL5 GENERIC CONFIRMATION SUMMARY =="
.venv-hyak/bin/python - <<'PY'
from pathlib import Path
import pandas as pd

root = Path("/gscratch/scrubbed/lei0603/ft-ppi/artifacts/upworthy_question_vader_full5_confirm_10rep_generic_1p5b")
...
winners = be[be["beats_labeled_only_proxy"] & be["method"].str.contains("ifvarq", case=False, na=False)].copy()
if winners.empty:
    print("none")
else:
