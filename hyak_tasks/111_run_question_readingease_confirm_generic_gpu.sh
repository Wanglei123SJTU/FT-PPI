...
TASK_ID="111_run_question_readingease_confirm_generic_gpu"
...
CONFIG="configs/upworthy_question_readingease_confirm_10rep_generic_1p5b.yaml"
ROOT="/gscratch/scrubbed/${USER}/ft-ppi/artifacts/upworthy_question_readingease_confirm_10rep_generic_1p5b"

echo "== CANCEL STALLED H200-SPECIFIC ARRAY IF STILL PRESENT =="
squeue -u "$USER" || true
if squeue -j 36220507 -h >/dev/null 2>&1 && [ -n "$(squeue -j 36220507 -h)" ]; then
  echo "cancelling_stalled_h200_job=36220507"
  scancel 36220507 || true
  sleep 10
fi
squeue -u "$USER" || true

echo "== DESCRIBE GENERIC GPU CONFIRM CONFIG =="
.venv-hyak/bin/python -m src.experiments.upworthy_question_scaling_law describe --config "$CONFIG"

echo "== RUN QUESTION + READING_EASE 10-REP CONFIRMATION ON GENERIC CKPT-G2 GPU =="
export CONFIG
...
echo "== QUESTION + READING_EASE GENERIC CONFIRMATION SUMMARY =="
.venv-hyak/bin/python - <<'PY'
from pathlib import Path
import pandas as pd

root = Path("/gscratch/scrubbed/lei0603/ft-ppi/artifacts/upworthy_question_readingease_confirm_10rep_generic_1p5b")
