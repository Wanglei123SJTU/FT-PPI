...
echo "wine_var_scaling_b10000_task_start"
hostname
date
...
while squeue -j "$JOB_ID" -h >/dev/null 2>&1 && [ -n "$(squeue -j "$JOB_ID" -h)" ]; do
  ...
  find logs -maxdepth 1 -name "wine-varscale-${JOB_ID}_*.out" -type f -printf "%T@ %p\n" 2>/dev/null | sort -n | tail -4 | awk '{print $2}' | while read -r log; do
    [ -n "$log" ] || continue
    echo "--- tail $log ---"
    tail -80 "$log" || true
  done
...
find logs -maxdepth 1 -name "wine-varscale-${JOB_ID}_*.out" -type f | sort | tail -10 | while read -r log; do
  echo "--- final tail $log ---"
  tail -120 "$log" || true
done
...
root = Path("artifacts/wine_var_scaling_b10000")
required = [
...
echo "wine_var_scaling_b10000_task_done"
