...
echo "resume_allocation_scaling_probe_task_start"
hostname
date
...
echo "== CLEAN LARGE INTERMEDIATE ARTIFACTS =="
if [ -d artifacts/allocation_scaling_probe ]; then
  du -sh artifacts/allocation_scaling_probe || true
  find artifacts/allocation_scaling_probe -type d \( -name final_adapter -o -name checkpoints \) -prune -print -exec rm -rf {} +
  du -sh artifacts/allocation_scaling_probe || true
else
  echo "artifacts/allocation_scaling_probe does not exist yet"
fi
...
echo "== SUBMIT RESUMED ALLOCATION SCALING PROBE =="
submit_output=""
submit_status=1
for args in \
...
if [ "$submit_status" -ne 0 ]; then
  echo "All resumed allocation scaling probe submit attempts failed."
  exit 1
fi
...
echo "resume_allocation_scaling_probe_task_done"
