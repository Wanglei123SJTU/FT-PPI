...
echo "helpsteer2_lora_scaling_pilot_x8_task_start $(date)"
...
OUTPUT_DIR="${OUTPUT_DIR:-/gscratch/scrubbed/${USER}/ft-ppi/artifacts/helpsteer2_lora_scaling_pilot_x8_137}"
...
S_GRID="${S_GRID:-50,100,150,200,300,500,700}"
...
echo "submitting scaling pilot: ${SBATCH_CMD[*]}"
SUBMIT_OUTPUT="$("${SBATCH_CMD[@]}")"
echo "$SUBMIT_OUTPUT"
JOB_ID="$(echo "$SUBMIT_OUTPUT" | awk '{print $NF}')"
echo "scaling_pilot_job_id=$JOB_ID"
...
while squeue -j "$JOB_ID" -h | grep -q .; do
  ...
  if [ "$completed" -eq 0 ] && [ "$elapsed" -gt 1800 ]; then
...
for log in $(ls -1t /gscratch/scrubbed/"$USER"/ft-ppi/logs/hs2-worker-"${JOB_ID}"_*.out 2>/dev/null | head -n 16); do
  echo "---- $log ----"
  tail -n 160 "$log" || true
done
...
if [ "$COMPLETED_CELLS" -lt "$N_CELLS" ]; then
  echo "missing cells; not aggregating full pilot" >&2
  exit 1
fi
...
echo "helpsteer2_lora_scaling_pilot_x8_task_done $(date)"
