...
echo "gpu_args=$GPU_ARGS array_concurrency=$ARRAY_CONCURRENCY model_name=$MODEL_NAME max_length=$MAX_LENGTH train_batch_size=$TRAIN_BATCH_SIZE eval_batch_size=$EVAL_BATCH_SIZE max_epochs=$MAX_EPOCHS patience=$PATIENCE no_gradient_checkpointing=$NO_GRADIENT_CHECKPOINTING"

SBATCH_CMD=(
  sbatch
  $GPU_ARGS
  --array=0-"$LAST_INDEX"%"$ARRAY_CONCURRENCY"
  --export=ALL,HYAK_VENV_DIR="$VENV_REAL",INPUT_CSV="$INPUT_CSV",OUTPUT_DIR="$OUTPUT_DIR",PLAN_CSV="$PLAN_CSV",FEATURES="$FEATURES",MODEL_NAME="$MODEL_NAME",MAX_LENGTH="$MAX_LENGTH",TRAIN_BATCH_SIZE="$TRAIN_BATCH_SIZE",EVAL_BATCH_SIZE="$EVAL_BATCH_SIZE",GRAD_ACCUM="$GRAD_ACCUM",MAX_EPOCHS="$MAX_EPOCHS",PATIENCE="$PATIENCE",NO_GRADIENT_CHECKPOINTING="$NO_GRADIENT_CHECKPOINTING",TRANSFORMERS_OFFLINE="$TRANSFORMERS_OFFLINE",HF_HUB_OFFLINE="$HF_HUB_OFFLINE"
