...
echo "mse_first_pilot_batch4_task_start"
hostname
date
...
echo "== SUBMIT MSE FIRST PILOT BATCH4 =="
submit_output=""
submit_status=1
JOB_ID=""
...
do
  echo "Trying: sbatch $args slurm/run_mse_first_pilot_batch4.sbatch"
  set +e
  submit_output=$(sbatch $args slurm/run_mse_first_pilot_batch4.sbatch 2>&1)
  submit_status=$?
  set -e
  echo "$submit_output"
...
if [ "$submit_status" -ne 0 ]; then
  echo "All MSE first pilot batch4 submit attempts failed."
  exit 1
fi
...
echo "JOB_ID=$JOB_ID"
JOB_LOG="logs/wine-b4pilot-${JOB_ID}.out"
...
root = Path("artifacts/mse_first_pilot_batch4")
...
for budget, train_sizes in expected.items():
    got = set(ppi_plus.loc[ppi_plus["budget"].astype(int) == budget, "train_size"].astype(int))
    if got != train_sizes:
        raise SystemExit(f"budget {budget} train sizes mismatch: got {sorted(got)}, expected {sorted(train_sizes)}")
...
print("mse first pilot batch4 ppi_plus")
print(
    ppi_plus[[
        "budget",
        "allocation_ratio",
        ...
        "mean_sample_savings",
        "mean_ci_length",
    ]].sort_values(["budget", "train_size"]).to_string(index=False)
)
print("mse first pilot batch4 rampup allocation regret")
print(regret.to_string(index=False))

target_diag = diagnostics[(diagnostics["role"].isin(["population", "correction"])) & (diagnostics["budget"].isin(expected))]
print("mse first pilot batch4 prediction diagnostics")
print(
    target_diag[[
        "budget",
        "loss",
        ...
        "mean_pred_std",
        "mean_residual_variance",
    ]].sort_values(["budget", "train_size", "role"]).to_string(index=False)
)
PY

echo "mse_first_pilot_batch4_task_done"
