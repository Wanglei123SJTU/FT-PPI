...
PYTHON_BIN="${HYAK_VENV_DIR:-.venv-hyak}/bin/python"
if [ ! -x "$PYTHON_BIN" ] && [ -x ".venv-hyak/bin/python" ]; then
  PYTHON_BIN=".venv-hyak/bin/python"
fi

echo "summarize_helpsteer2_lora_partial_venv_start $(date)"
...
echo "summarize_helpsteer2_lora_partial_venv_done $(date)"
