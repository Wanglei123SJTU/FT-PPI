...
echo "training_signal_probe_task_start"
hostname
date
...
echo "== SUBMIT TRAINING SIGNAL PROBE =="
submit_output=""
submit_status=1
for args in \
...
do
  echo "Trying: sbatch $args slurm/run_training_signal_probe.sbatch"
  set +e
  submit_output=$(sbatch $args slurm/run_training_signal_probe.sbatch 2>&1)
  submit_status=$?
  set -e
  echo "$submit_output"
...
if [ "$submit_status" -ne 0 ]; then
  echo "All training signal probe submit attempts failed."
  exit 1
fi
...
echo "JOB_ID=$JOB_ID"
JOB_LOG="logs/wine-signal-${JOB_ID}.out"
...
root = Path("artifacts/training_signal_probe")
...
if diagnostics["variant"].nunique() != 4:
    raise SystemExit("missing variants in diagnostics")

pd.set_option("display.max_columns", None)
pd.set_option("display.width", 220)
print("training signal diagnostics rows=", len(diagnostics))
...
print("training signal ppi_plus")
print(ppi.to_string(index=False))
PY

echo "training_signal_probe_task_done"
