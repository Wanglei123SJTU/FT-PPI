...
TASK_ID="109_run_question_readingease_confirm_10rep"
...
CONFIG="configs/upworthy_question_readingease_confirm_10rep_1p5b.yaml"
ROOT="/gscratch/scrubbed/${USER}/ft-ppi/artifacts/upworthy_question_readingease_confirm_10rep_1p5b"

echo "== CURRENT QUEUE BEFORE CONFIRM RUN =="
squeue -u "$USER" || true

echo "== DESCRIBE QUESTION + READING_EASE CONFIRM CONFIG =="
.venv-hyak/bin/python -m src.experiments.upworthy_question_scaling_law describe --config "$CONFIG"

echo "== RUN QUESTION + READING_EASE 10-REP CONFIRMATION =="
export CONFIG
...
echo "== QUESTION + READING_EASE CONFIRMATION SUMMARY =="
.venv-hyak/bin/python - <<'PY'
from pathlib import Path
import pandas as pd

root = Path("/gscratch/scrubbed/lei0603/ft-ppi/artifacts/upworthy_question_readingease_confirm_10rep_1p5b")
