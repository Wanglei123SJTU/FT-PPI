...
echo "allocation_diagnostic_task_start"
hostname
date
...
echo "== SUBMIT ALLOCATION DIAGNOSTIC =="
submit_output=""
submit_status=1
for args in \
...
do
  echo "Trying: sbatch $args slurm/run_allocation_diagnostic.sbatch"
  set +e
  submit_output=$(sbatch $args slurm/run_allocation_diagnostic.sbatch 2>&1)
  submit_status=$?
  set -e
  echo "$submit_output"
...
if [ "$submit_status" -ne 0 ]; then
  echo "All allocation diagnostic submit attempts failed."
  exit 1
fi
...
echo "JOB_ID=$JOB_ID"
JOB_LOG="logs/wine-alloc-${JOB_ID}.out"
...
echo "== OUTPUTS =="
find artifacts/allocation_diagnostic -maxdepth 5 -type f 2>/dev/null | sort || true
...
root = Path("artifacts/allocation_diagnostic")
...
if diagnostics.empty or all_metrics.empty:
    raise SystemExit("allocation diagnostics are empty")
...
missing_cols = required_cols.difference(diagnostics.columns)
if missing_cols:
    raise SystemExit("diagnostic_summary.csv missing columns: " + ", ".join(sorted(missing_cols)))

expected_train_sizes = {50, 100, 200}
observed_train_sizes = set(diagnostics["train_size"].astype(int))
if not expected_train_sizes.issubset(observed_train_sizes):
    raise SystemExit(f"missing train sizes: {sorted(expected_train_sizes - observed_train_sizes)}")
...
for tag in ["s0050_v0100", "s0100_v0100", "s0200_v0100"]:
    for loss in ["mse", "var"]:
        ...
        if df.empty:
            raise SystemExit(f"{pred} is empty")
...
print(diagnostics[["allocation_ratio", "train_size", "loss", "method", "residual_variance", "estimated_estimator_variance", "ci_length"]])
PY

echo "allocation_diagnostic_task_done"
