...
echo "helpsteer2_lora_fixed_eval_large_batch_retry_task_start $(date)"

cd ~/FT-PPI
export PYTHONPATH="$PWD:${PYTHONPATH:-}"

RUN_NAME="helpsteer2_lora_fixed_eval_large_batch_141"
OUTPUT_DIR="/gscratch/scrubbed/${USER}/ft-ppi/artifacts/${RUN_NAME}"
INPUT_CSV="${OUTPUT_DIR}/helpsteer2_preference_pairs.csv"
SOURCE_CSV="/gscratch/scrubbed/${USER}/ft-ppi/artifacts/helpsteer2_lora_scaling_pilot_x8_137/helpsteer2_preference_pairs.csv"
...
if [ ! -f "$INPUT_CSV" ]; then
  if [ -f "$SOURCE_CSV" ]; then
    echo "copying input csv from $SOURCE_CSV"
...
echo "submitting fixed-eval retry: ${SBATCH_CMD[*]}"
SUBMIT_OUTPUT="$("${SBATCH_CMD[@]}")"
echo "$SUBMIT_OUTPUT"
JOB_ID="$(echo "$SUBMIT_OUTPUT" | awk '{print $NF}')"
echo "fixed_eval_retry_job_id=$JOB_ID"
...
if [ "$COMPLETED_CELLS" -lt "$N_CELLS" ]; then
  echo "missing cells; not aggregating fixed-eval retry" >&2
  exit 1
fi
...
echo "report preview"
sed -n '1,240p' "$OUTPUT_DIR/lora_scaling_report.md"

echo "helpsteer2_lora_fixed_eval_large_batch_retry_task_done $(date)"
