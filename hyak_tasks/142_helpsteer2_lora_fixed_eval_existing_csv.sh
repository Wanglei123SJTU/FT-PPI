...
echo "helpsteer2_lora_fixed_eval_existing_csv_task_start $(date)"

cd ~/FT-PPI
export PYTHONPATH="$PWD:${PYTHONPATH:-}"

RUN_NAME="helpsteer2_lora_fixed_eval_existing_csv_142"
...
if [ ! -s "$INPUT_CSV" ]; then
  echo "missing existing input csv: $INPUT_CSV" >&2
...
echo "submitting fixed-eval existing-csv: ${SBATCH_CMD[*]}"
SUBMIT_OUTPUT="$("${SBATCH_CMD[@]}")"
echo "$SUBMIT_OUTPUT"
JOB_ID="$(echo "$SUBMIT_OUTPUT" | awk '{print $NF}')"
echo "fixed_eval_existing_csv_job_id=$JOB_ID"
...
if [ "$COMPLETED_CELLS" -lt "$N_CELLS" ]; then
  echo "missing cells; not aggregating fixed-eval existing-csv" >&2
  exit 1
fi
...
echo "helpsteer2_lora_fixed_eval_existing_csv_task_done $(date)"
