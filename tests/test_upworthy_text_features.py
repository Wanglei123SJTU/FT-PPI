...
from src.data.upworthy_text_features import (
    ...
    CONTINUOUS_FEATURES,
    FEATURES,
    add_pair_differences,
    add_pair_features_from_headlines,
    construct_arm_features,
    raw_headline_features,
)
...
def test_raw_headline_features_detect_basic_cues():
    ...
    assert feats["has_second_person"] == 1
    assert feats["avg_word_length"] > 0
    assert feats["common_word_score"] > 0
    assert "reading_ease" in feats
    assert "vader_compound" in feats
...
def test_construct_arm_features_contains_requested_constructs():
    ...
    assert out.loc[0, "SECOND_PERSON"] == 1
...
def test_add_pair_differences_uses_a_minus_b():
    ...
    for feature in CONTINUOUS_FEATURES:
        assert f"delta_{feature}_scale" in out


def test_continuous_pair_differences_scale_without_centering():
    arms = pd.DataFrame(
        {
            "test_id": [1, 1],
            "arm_id": [0, 1],
            "headline": ["One two three four five six seven", "One two"],
        }
    )
    arm_features = construct_arm_features(arms)
    forward = pd.DataFrame({"pair_id": [0], "test_id": [1], "arm_id_a": [0], "arm_id_b": [1]})
    backward = pd.DataFrame({"pair_id": [0], "test_id": [1], "arm_id_a": [1], "arm_id_b": [0]})
    fwd = add_pair_differences(forward, arm_features)
    bwd = add_pair_differences(backward, arm_features)
    for feature in CONTINUOUS_FEATURES:
        assert np.isclose(fwd[f"delta_{feature}"].iloc[0], -bwd[f"delta_{feature}"].iloc[0])
        if not np.isclose(fwd[f"delta_{feature}_raw"].iloc[0], 0.0):
            assert not np.isclose(fwd[f"delta_{feature}"].iloc[0], 0.0)


def test_pair_headline_enrichment_rebuilds_stale_feature_columns():
    pairs = pd.DataFrame(
        {
            "pair_id": [0],
            "test_id": [1],
            "arm_id_a": [0],
            "arm_id_b": [1],
            "headline_a": ["Can You Name 3 Reasons This Works?"],
            "headline_b": ["A Simple Story About People"],
            "delta_QUESTION": [999.0],
            "QUESTION_a": [999.0],
            "QUESTION_b": [999.0],
        }
    )
    out = add_pair_features_from_headlines(pairs)
    assert out["delta_QUESTION"].iloc[0] == 1.0
    assert out["QUESTION_a"].iloc[0] == 1.0
    assert out["QUESTION_b"].iloc[0] == 0.0
